...
# Repository root (contains the canonical VERSION file). ROOT_DIR is the
# deployment/upgrade dir (holds deploy.env, compose/, .state) — the repo root is
# two levels up. Prefer `git rev-parse --show-toplevel` (robust to any nesting);
# fall back to ROOT_DIR/../.. when git is unavailable. Never hardcodes a version.
REPO_ROOT="$(git -C "$ROOT_DIR" rev-parse --show-toplevel 2>/dev/null || true)"
[ -n "$REPO_ROOT" ] || REPO_ROOT="$(cd "$ROOT_DIR/../.." && pwd)"
