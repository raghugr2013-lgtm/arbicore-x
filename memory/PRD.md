...
## Done — 2026-06 (STAGE 2 — production dependency resolution FIXED)
- ROOT CAUSE: `app/backend/requirements.txt` pinned `emergentintegrations==0.2.0`, an Emergent-platform-ONLY package (private cloudfront index, NOT on public PyPI). The canonical prod Dockerfile installs that manifest → VPS `pip install` failed ("No matching distribution"). Classification: (b) platform-only + (d) stale for VPS.
- VERIFIED unused: zero `import emergentintegrations` / `from emergentintegrations` in production code (grep across app/backend excluding tests; no dynamic import). The SHADOW arbitrage app uses no LLM.
- FIX: surgically removed the single `emergentintegrations==0.2.0` line from `app/backend/requirements.txt` (no other manifest changes; all other deps are standard pinned PyPI). Not weakening the app; not manually installing on VPS; not adding a private index to the prod build.
- All prior Stage-2 fixes preserved. No app/scanner/execution/safety logic changed; no Mongo data touched. Backend stays healthy (200); package remains locally installed but unimported.
- Tests: `tests/test_stage2_prod_dependencies.py` (NEW) 4/4 — no platform-only pkg, no VCS/URL/local/private-index entries, all entries pinned, removed pkg not imported. All Stage-2 suites 29/29.

## Done — 2026-06 (STAGE 2 — canonical build context FIXED)
- ROOT CAUSE: prod compose backend `build.context: ../backend` = `deployment/upgrade/backend/` which contains only Dockerfile + requirements.prod.txt — NOT `requirements.txt` and NOT the app source. So `COPY requirements.txt /app/requirements.txt` failed (`/requirements.txt not found`), and `COPY . /app` would have shipped no app code.
- FIX: `deployment/upgrade/compose/docker-compose.prod.yml` backend build now uses `context: ../../../app/backend` (real source: has requirements.txt + server.py + arbicore/) and `dockerfile: ../../deployment/upgrade/backend/Dockerfile` (canonical Dockerfile referenced relative to context; BuildKit/compose v2). Added `app/backend/.dockerignore` (excludes .env, __pycache__, tests, *.pyc) so secrets/cruft aren't baked. No generated requirements copied to VPS; the app's own committed `app/backend/requirements.txt` is the consumed manifest.
- All prior Stage-2 fixes preserved (Mongo selection, MONGO_URL reconciliation, writable state dirs, deploy.env serialization, metadata propagation, env parity). No app/scanner/execution/safety logic; no Mongo data touched.
- Tests: `tests/test_stage2_build_context.py` (NEW) 4/4 — context contains requirements.txt+server.py+arbicore/, dockerfile ref resolves to canonical upgrade Dockerfile, all concrete COPY sources satisfiable from context, .dockerignore present excluding .env. All Stage-2 suites 25/25. Compose YAML valid.

## Done — 2026-06 (STAGE 2 — deploy.env shell-safe serialization FIXED)
- ROOT CAUSE: `00_detect_env.sh` wrote `deploy.env` via a raw `${VAR}` heredoc, so `MONGO_MOUNTS` (e.g. `volume:/…/_data->/data/configdb; volume:/…/_data->/data/db;`) was unquoted. When `05_build.sh` sourced deploy.env (`load_env`), the `;` split it into commands → `/data/configdb: No such file or directory`.
- FIX: added pure `env_kv KEY VALUE` serializer to `common.sh` (single-quote wrapping + `'\''` escaping → semicolons/paths/`->`/spaces inert when sourced, value preserved exactly). `00_detect_env.sh` now builds deploy.env with `env_kv` for every key instead of the heredoc. compose/.env unchanged (consumed by `docker compose --env-file`, not shell-sourced).
- All prior Stage-2 fixes preserved (Mongo selection, MONGO_URL reconciliation, user-writable state dirs, metadata chain, env parity). No app/scanner/execution/safety logic; no Mongo data touched.
- Tests: `tests/test_stage2_deploy_env_serialization.py` (NEW) 4/4 — multi-mount sourced inertly + preserved, single-quoted format, embedded-quote preserved, deploy.env uses env_kv not raw heredoc. All Stage-2 suites 21/21. `bash -n` clean; end-to-end simulation of the exact failing value passes.

## Done — 2026-06 (STAGE 2 — permission + MONGO_URL reconciliation FIXED)
- ISSUE 1 (root-only /opt): `BACKUP_DIR`/`LOG_DIR` defaulted to `/opt/arbicore-x/*` → `mkdir` failed for the `raghu` deploy user. FIX in `common.sh`: default to user-writable `${ARBICORE_STATE_DIR:-$ROOT_DIR/.state}/{backups,logs}`; explicit `BACKUP_DIR`/`LOG_DIR`/`ARBICORE_STATE_DIR` overrides still honored. Added `deployment/upgrade/.gitignore` (.state/, generated env files).
- ISSUE 2 (wrong Mongo host): `00_detect_env.sh` line 59 inherited the OLD backend's MONGO_URL (host `factory-mongo`) even though the preflight selected `arbicore-x-mongo`. FIX: added pure `mongo_url_host` + `rewrite_mongo_host` helpers to `common.sh`; `00_detect_env.sh` now reconciles the URL host to the SELECTED authoritative container (preserving scheme/creds/authSource/port/path/query), with a loud WARN when the old host differed and an `ARBICORE_MONGO_URL` full-override + a post-condition asserting url_host == selected container. Verified: old `factory-mongo` → reconciled `arbicore-x-mongo` (creds preserved).
- No app/scanner/opportunity/execution/safety logic changed; no Mongo data touched; Stage-2 metadata chain intact.
- Tests: `tests/test_stage2_env_reconciliation.py` (NEW) 7/7 — host rewrite (creds/query/path preserved), host extraction, reconciled==selected, state dirs not /opt + override honored. Total Stage-2 tests 17/17. Shell `bash -n` clean; end-to-end VPS-case simulation passes.

## Done — 2026-06 (STAGE 2 — multi-Mongo preflight blocker FIXED)
- ROOT CAUSE: `00_detect_env.sh` hard-required exactly 1 Mongo container; the VPS has 4 (arbicore-x-mongo, factory-mongo, foreman-cert-mongo, foreman-mongo) → it aborted before generating `compose/.env`, so `05_build.sh` failed with `IMAGE_TAG: unbound variable`.
- FIX (canonical, Git only, no data touched): added pure `choose_mongo_container` (explicit override → single candidate → authoritative default `ARBICORE_DEFAULT_MONGO=arbicore-x-mongo`) + `validate_mongo_container` (running + not-unhealthy) to `deployment/upgrade/lib/common.sh`; rewired `00_detect_env.sh` Mongo detection to use them (explicit from `MONGO_CONTAINER` env or existing deploy.env; clear die messages for not-found/ambiguous). `MONGO_CONTAINER` still persisted to deploy.env. Stage-2 metadata chain (GITSHA/GITTAG/BUILD_TIME/APP_VERSION → compose/.env → build.args → Dockerfile ARG→ENV → /api/version) untouched.
- No app/runtime/scanner/execution/safety logic changed. No Mongo data altered.
- Test: `tests/test_stage2_mongo_selection.py` (NEW, docker-free) — exact 4-Mongo VPS case picks arbicore-x-mongo; explicit override; not-found→rc2; single candidate; ambiguous-no-default→rc3. 6/6 pass (+ metadata test 4/4). Verified end-to-end selector on VPS layout returns arbicore-x-mongo rc=0; both shell files `bash -n` clean.

## Done — 2026-06 (STAGE 2 — deployment metadata propagation FIXED)
- ROOT CAUSE: the canonical upgrade build uses `deployment/upgrade/compose/docker-compose.prod.yml` + `deployment/upgrade/backend/Dockerfile` (NOT the `deployment/docker/...` files edited in Stage 1). The prod compose backend `build:` declared NO `args:` (only a gitsha label) and the upgrade Dockerfile had NO `ARG GITSHA/GITTAG/BUILD_TIME/APP_VERSION` nor `ARBICORE_*` ENV → Dockerfile ARGs defaulted → `/api/version` reported unknown/unset.
- FIX (canonical, Git only): (1) `deployment/upgrade/backend/Dockerfile` — added `ARG GITSHA/GITTAG/BUILD_TIME/APP_VERSION` + `ENV ARBICORE_GIT_SHA/GIT_TAG/BUILD_TIME/VERSION` mapped to them. (2) `deployment/upgrade/compose/docker-compose.prod.yml` — added `build.args` passing `${GITSHA}/${GITTAG}/${BUILD_TIME}/${APP_VERSION}`. (3) `deployment/upgrade/steps/00_detect_env.sh` already writes all 4 into `compose/.env` (Stage-1 edit). Chain: 00_detect_env→compose/.env → prod compose build.args → Dockerfile ARG→ENV → `/api/arbicore/version`.
- No auth/scanner/opportunity/gate/execution/safety logic touched.
- Test: `tests/test_stage2_deployment_metadata.py` (NEW) — static propagation guard (compose args, Dockerfile ARG→ENV, detect_env heredoc) + endpoint contract. 4/4 pass. Runtime read of ARBICORE_* env verified.

## Done — 2026-06 (STAGE 1 — canonical real FlashLoanArbitrageScanner activated)
- Wired the REAL `FlashLoanArbitrageScanner` as the canonical flash-loan discovery plane (supersedes the dormant wave1b `ShadowScannerAdapter`). Files: `arbicore/scanners/flash_loan_arbitrage/live_quote_provider.py` (NEW — live quote provider over `_QUOTER_REGISTRY`, honest gross_profit from real on-chain quotes, returns None when unpriceable), `arbicore/runtime/composition.py` (`get_flash_loan_arb_scanner` now uses the REAL Base pool universe via `build_pool_graph()`, detection enabled, Base+Balancer V2 cfg; new `activate_canonical_flash_loan_scanner(quoter_registry)`), `server.py` (`_canonical_flash_loan_scanner_startup` + `/scanners/status` now reports `canonical_flash_loan_arbitrage` instantiated/live/authoritative and marks the shadow adapter non-authoritative).
- Path: discovery (RouteSearchEngine over 30 real Base pools) → live quoting (QuoterRegistry) → economics/EV → Gate7 atomic-profit + Gate8 liquidity + Gate9 MEV → EmissionBus → pipeline → AutoExecutor (mode-gated). Detection-only/SHADOW; never signs/broadcasts.
- Verified live: 32 real route cycles claimed in one tick; 18 denied by Gate7 (unprofitable), 14 unpriceable; 0 emitted (honest NO QUALIFYING OPPORTUNITY — no fabrication, gates preserved), 0 errors.
- Deployment identity: enriched `deployment/upgrade/steps/00_detect_env.sh` to write GITTAG/BUILD_TIME/APP_VERSION into compose/.env; backend Dockerfile ENV already maps GITSHA→ARBICORE_GIT_SHA so `/api/arbicore/version` reports the canonical build metadata in the production image.
- Tests: `tests/test_stage1_canonical_flash_loan_scanner.py` (NEW) + executor-verify + auth-provisioning + flash-loan subsystem (route_search/economics/gates/verifiers) = all pass (STAGE1 9/9; subsystem 156/156; auth+scanner 47/47). No regressions.

## Done — 2026-06 (Auth source-of-truth drift FIXED canonically — v2.9.4)
- Canonical auth = `users` collection + `routes/auth.py`/`services/auth.py` (bcrypt+JWT cookie/bearer+lockout). `_resolve_current_user` already prefers it; legacy `arbicore/auth` (`auth_users`) stays gated OFF as read-only bearer fallback only.
- ROOT CAUSE: canonical auth only created a user via interactive `/api/auth/setup`; no startup provisioning → fresh prod DB had 0 users → `/api/auth/login` 401 despite `ARBICORE_ADMIN_USER/PASS` configured.
- FIX: added `services.auth.ensure_provisioned_users()` — idempotent, insert-only startup seeder into the SAME `users` collection login reads; seeds admin (+ operator if its env creds present) from `ARBICORE_ADMIN_USER/ARBICORE_ADMIN_PASS` (+ `ARBICORE_OPERATOR_USER/PASS`); never overwrites an existing user's password; ensures unique username index; DuplicateKeyError-safe under concurrency; skips gracefully + warns when creds/`JWT_SECRET` absent (no crash, no weak default). Wired as `@app.on_event("startup")` `_canonical_auth_provision_startup`.
- Verified live: boot log created admin once then `existed` on re-boot (idempotent); login correct→200 + `/me`→200; wrong→401. Tests: `tests/test_auth_canonical_provisioning.py` 7/7 PASS incl. drift detector J (seed collection == login collection == `users`), idempotency, password-preservation, configured-password-logs-in.
- Files changed: `services/auth.py` (+seeder), `server.py` (+startup event, +/api/arbicore/version earlier), `tests/test_auth_canonical_provisioning.py` (new). Creds in `/app/memory/test_credentials.md`.

## Done — 2026-06 (Production completion pass — canonical freeze + drift fix + deploy identity)
- **SOFTWARE BUG FIXED — executor-verify drift**: created `arbicore/execution/executor_interface.py` (single source of truth: `VAULT()`/`ROUTER()`/`owner()`, entrypoint `execute(address[],uint256[],bytes)`). Reconciled `operator_wizard.verify_executor` to it (was probing non-existent `balancerVault()/uniRouter()/aavePool()` → wrongly BLOCKED). Verified getters resolve to Balancer Vault `0xBA12…2C8` + UniV3 router `0x2626…481` → `executor_verified` READY on a healthy RPC. Aave demoted to non-blocking INFO.
- **Deployment identity**: added `GET /api/arbicore/version` (git sha/tag/image digest/build time/runtime env; no secrets). Baked `ARBICORE_GIT_SHA/GIT_TAG/BUILD_TIME/VERSION` into backend Dockerfile + compose build args.
- **Canonical path frozen**: `arbicore/CANONICAL_PATH.md` — active plane = ContinuousScanner→OpportunityEngine + AutoExecutor→OpportunityPipeline (policy+mode gated). Legacy generations (wave1b adapters, composition family scanners gated by RUNTIME_AUTOSTART=off, scanners/live/*, aerodrome, Aave-Sepolia verify) marked KEEP-FROZEN/DEPRECATED/REMOVED.
- **Stale tests reconciled** (3): test_executor_verify_live.py rewritten to canonical interface; test_arbicore_opportunity_probe + test_technical_validation_iter3 fixed (one encoded the drift as BLOCKED, one hard-coded the retired Sepolia executor). All pass.
- **Autonomous model confirmed already wired** (no rebuild): capital_policy hard limits + `PATCH /execution/capital-policy/{strategy}` + mode ladder + AutoExecutor enforce policy & mode; no per-opp approval; broadcast blocked below LIMITED_LIVE.
- Tests: curated deterministic acceptance = 118 passed; full-suite preview 1813 passed, ~350 failures are ENV (auth brute-force lockout 429 + public-RPC 429/timeouts under 2000-test concurrency), not code — pass on VPS/in isolation.
- Acceptance report: `/app/deploy/ARBICORE_X_PRODUCTION_ACCEPTANCE.md` (categorized SOFTWARE BUG / CONFIG REQUIRED / MARKET-DEPENDENT / SAFETY-LOCKED). SHADOW/PAPER path is software-complete & deployment-ready; live gated only by market + operator authorizations. No safety gate weakened.
...
## Done — 2026-06 (Scanner bootstrap decoupling + validator-contract fix — post-deploy validation GREEN-able)
- ROOT CAUSE of `10_validate.sh` post-deploy failure (3 linked issues): (1) scanner_config/scanner_state seeding + arbicore_* TTL index bootstrap were wrongly gated inside `_arbicore_runtime_autostart()` which returns early when `ARBICORE_RUNTIME_AUTOSTART` is off (safe SHADOW default) → 0 config / 0 state docs, missing ttl_30d/ttl_90d; (2) `scanners_router` was never registered → `/api/arbicore/scanners/cex_arb/config` absent from openapi; (3) `04_validate.js` demanded cex_arb/funding_arb `enabled===true` + `recent verifications > 0`, contradicting the dormant SHADOW/PAPER posture.
- FIX (additive, safety-preserving): `server.py` — new ALWAYS-RUN startup handler `_arbicore_bootstrap_substrate` does index bootstrap + `cfg_repo.seed_defaults()` + `state_repo.seed_defaults()` (idempotent, insert-if-absent); the per-scanner instantiate+`start()` loop stays gated behind `ARBICORE_RUNTIME_AUTOSTART` + `ARBICORE_SCANNER_*`. Registered the canonical `scanners_router` (committing the operator's manual VPS edit) inside a try/except mirroring the auth-router pattern. `04_validate.js` — asserts scanner state ROWS exist (dormant enabled=false is correct), makes recent-verifications informational; kept 6+6 doc-count + TTL-index checks.
- SAFETY: nothing enabled — `ARBICORE_RUNTIME_AUTOSTART` still unset, all 6 state rows seed `enabled=False`, no scanner started (autostart handler done 0.00s), mode stays SHADOW, LIMITED_LIVE + FULL_AUTOMATION untouched, no signer/broadcast/capital path touched. New scanner endpoints are auth-gated (401 verified unauth).
- VERIFIED live (preview Mongo): 6 arbicore_scanner_config + 6 arbicore_scanner_state docs, every state enabled=False; ttl_30d + ttl_90d present; openapi exposes `/api/arbicore/scanners/cex_arb/config`. Tests: NEW `tests/test_scanner_bootstrap_decoupled.py` 4/4; substrate+stage1 regression 45/45. JS + shell `--check`/`bash -n` clean.
