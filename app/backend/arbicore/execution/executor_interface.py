...
The deployed FlashLoanReceiver on Base exposes the canonical public
getters ``balancerVault()``, ``aavePool()``, ``uniRouter()`` and ``owner()``,
plus the Balancer entrypoint ``execute(address[],uint256[],bytes)`` and the
Aave entrypoint ``executeAave(address,uint256,bytes)``.

Every executor verification path — the operator wizard, fork validation,
execution-capability checks and atomic simulation — MUST read this module
instead of hard-coding its own selectors.

This module is the Python-side ABI source of truth for the deployed
FlashLoanReceiver. The historical ``VAULT()/ROUTER()`` assumptions were stale
and caused executor verification to disagree with the actual deployed
contract.

Verified against ``contracts/contracts/core/FlashLoanReceiver.sol`` and the
compiled ``FlashLoanReceiver`` ABI.
"""
...
# --- Public getters present on the deployed executor ----------------------- #
GETTER_VAULT_SIG = "balancerVault()"  # 0x158274a5 → Balancer V2 Vault
GETTER_AAVE_SIG = "aavePool()"        # 0xa03e4bc3 → Aave V3 Pool
...
SEL_AAVE = selector(GETTER_AAVE_SIG)
...
# The deployed receiver supports both canonical flash-loan heads.
FLASH_PROVIDERS = ("balancer_v2", "aave_v3")
...
__all__ = [
    ...
    "GETTER_VAULT_SIG", "GETTER_AAVE_SIG",
    "GETTER_ROUTER_SIG", "GETTER_OWNER_SIG",
    ...
    "FLASH_PROVIDERS", "FLASH_PROVIDER", "SWAP_VENUE",
]
