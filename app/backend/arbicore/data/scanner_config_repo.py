"""ArbiCore X — Phase D D-1: scanner_config + scanner_state repos."""
...
class ScannerConfigRepository:
    def __init__(self, db) -> None:
        self._col = db[CONFIG_COLLECTION]
        self._cache: Dict[str, Dict[str, Any]] = {}
        self._cache_ts: Dict[str, float] = {}

    async def ensure_indexes(self) -> None:
        # _id is implicitly unique in MongoDB — no explicit index needed.
        pass

    async def seed_defaults(self) -> None:
        existing = await self._col.find_one({"_id": "cex_arb"})
        if existing is None:
            await self._col.insert_one(dict(DEFAULT_CEX_ARB_CONFIG))
        existing_f = await self._col.find_one({"_id": "funding_arb"})
        if existing_f is None:
            await self._col.insert_one(dict(DEFAULT_FUNDING_ARB_CONFIG))
        # D-3.0 substrate seeding — config only; scanner module lands D-3.4.
        existing_d = await self._col.find_one({"_id": "dex_arb"})
        if existing_d is None:
            await self._col.insert_one(dict(DEFAULT_DEX_ARB_CONFIG))
        # D-4.0 substrate seeding — config only; orchestrator lands D-4.5.
        existing_l = await self._col.find_one({"_id": "launch_arb"})
        if existing_l is None:
            await self._col.insert_one(dict(DEFAULT_LAUNCH_ARB_CONFIG))
        # D-5.0 substrate seeding — config only; orchestrator lands D-5.5.
        # Operator-scope decisions baked in: LI.FI + Stargate bridges only,
        # 6 chains (ETH/Arbitrum/Base/Optimism/Polygon/Solana). Both
        # bridge and per-chain enable flags default False.
        existing_x = await self._col.find_one({"_id": "cross_chain_arb"})
        if existing_x is None:
            await self._col.insert_one(dict(DEFAULT_CROSS_CHAIN_ARB_CONFIG))
        # D-6.0 substrate seeding — flash-loan detection framework. Boot
        # posture DORMANT (every provider + chain flag is False). Operator
        # graduates via /api/arbicore/scanners/flash_loan_arb/* routes
        # (lands at D-6.1).
        existing_fl = await self._col.find_one({"_id": "flash_loan_arb"})
        if existing_fl is None:
            await self._col.insert_one(dict(DEFAULT_FLASH_LOAN_ARB_CONFIG))

    async def get(self, scanner_id: str = "cex_arb") -> Dict[str, Any]:
        # 60s cache
        now = time.time()
        if scanner_id in self._cache and (now - self._cache_ts.get(scanner_id, 0)) < 60:
            return self._cache[scanner_id]
        doc = await self._col.find_one({"_id": scanner_id})
        if doc is None:
            if scanner_id == "funding_arb":
                return dict(DEFAULT_FUNDING_ARB_CONFIG)
            if scanner_id == "dex_arb":
                return dict(DEFAULT_DEX_ARB_CONFIG)
            if scanner_id == "launch_arb":
                return dict(DEFAULT_LAUNCH_ARB_CONFIG)
            if scanner_id == "cross_chain_arb":
                return dict(DEFAULT_CROSS_CHAIN_ARB_CONFIG)
            if scanner_id == "flash_loan_arb":
                return dict(DEFAULT_FLASH_LOAN_ARB_CONFIG)
            return dict(DEFAULT_CEX_ARB_CONFIG)
        doc.pop("_id", None)
        self._cache[scanner_id] = doc
        self._cache_ts[scanner_id] = now
        return doc

    async def update(self, scanner_id: str, patch: Dict[str, Any]) -> Dict[str, Any]:
        await self._col.update_one(
            {"_id": scanner_id},
            {"$set": {**patch, "updated_at_ts": time.time()}},
            upsert=True,
        )
        self._cache.pop(scanner_id, None)
        return await self.get(scanner_id)


class ScannerStateRepository:
    def __init__(self, db) -> None:
        self._col = db[STATE_COLLECTION]
    ...
    async def seed_defaults(self) -> None:
        existing = await self._col.find_one({"_id": "cex_arb"})
        if existing is None:
            await self._col.insert_one({"_id": "cex_arb", "enabled": False})
        existing_f = await self._col.find_one({"_id": "funding_arb"})
        if existing_f is None:
            await self._col.insert_one({"_id": "funding_arb", "enabled": False})
        # D-3.0 substrate seeding — scanner state row only; the actual
        # DEXArbitrageScanner orchestrator does not exist yet (lands D-3.4).
        # `enabled=False` keeps it dormant until D-3.4 wires the factory and
        # the operator explicitly resumes via /api/arbicore/scanners/dex_arb/resume.
        existing_d = await self._col.find_one({"_id": "dex_arb"})
        if existing_d is None:
            await self._col.insert_one({"_id": "dex_arb", "enabled": False})
        # D-4.0 substrate seeding — scanner state row only; the actual
        # LaunchArbitrageScanner orchestrator does not exist yet (lands D-4.5).
        # `enabled=False` keeps it dormant until the orchestrator ships and
        # the operator explicitly resumes via /api/arbicore/scanners/launch_arb/resume.
        existing_l = await self._col.find_one({"_id": "launch_arb"})
        if existing_l is None:
            await self._col.insert_one({"_id": "launch_arb", "enabled": False})
        # D-5.0 substrate seeding — scanner state row only; the actual
        # CrossChainArbitrageScanner orchestrator does not exist yet (lands D-5.5).
        # `enabled=False` keeps it dormant until the orchestrator ships and
        # the operator explicitly resumes via /api/arbicore/scanners/cross_chain_arb/resume.
        existing_x = await self._col.find_one({"_id": "cross_chain_arb"})
        if existing_x is None:
            await self._col.insert_one(
                {"_id": "cross_chain_arb", "enabled": False})
        # D-6.0 substrate seeding — flash-loan detection framework. Same
        # dormant posture as D-5.0.
        existing_fl = await self._col.find_one({"_id": "flash_loan_arb"})
        if existing_fl is None:
            await self._col.insert_one(
                {"_id": "flash_loan_arb", "enabled": False})
    ...
    async def set_enabled(self, scanner_id: str, enabled: bool,
                          actor: str = "admin") -> Dict[str, Any]:
        await self._col.update_one(
            {"_id": scanner_id},
            {"$set": {"enabled": enabled, "updated_at_ts": time.time(),
                      "updated_by": actor}},
            upsert=True,
        )
        return await self.get(scanner_id)
