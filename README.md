...
**Version:** `v2.0.0` · **License:** Proprietary — All Rights Reserved

> **v2.0.0 is a canonical consolidation** — the previous `arbicore-x` v1.0.2 (deployment-complete) and the `Arbicorex-ui-v2-slice-02` slice (UI v2 + Wave 6 + Phase 7–10) have been merged into this single production repository.
> See [`docs/V2_CONSOLIDATION_AUDIT.md`](docs/V2_CONSOLIDATION_AUDIT.md) for the subsystem-by-subsystem audit and [`docs/V2_MIGRATION_GUIDE.md`](docs/V2_MIGRATION_GUIDE.md) for the fresh-VPS / upgrade migration flow.
> Regression state at v2.0.0 tag: **1442 tests passed, 76 skipped, 0 failed.**
...
- **`scripts/`** — the small handful of operator-facing entrypoints (`install.sh`, `upgrade.sh`, `healthcheck.sh`, `backup.sh`, `restore.sh`, `verify-deployment.sh`).
...
| **Deployment verification (v1.0.2+)** | `make verify DOMAIN=<url> PROFILE=<shared\|greenfield>` |
